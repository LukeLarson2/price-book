...
const SettingsPersonalize = ({ defaultImages }) => {
  ...
  const handleSaveChanges = () => {
    ...
    axios.put('/users/update-background', { userId: userData._id, backgroundImage: selectedImage })
    .then((response) => {
...
const handleFileChange = (e) => {
  ...
  axios.post('/users/upload-background-image', formData)
  .then((response) => {
    setModalMessage(response.data.message);
    setModalIsOpen(true);
    ...
    if (response.data.imagePath) {
      ...
      // update the userData
      setUserData({...userData, backgroundImage: response.data.imagePath });
    }
  })
    .catch((error) => console.error(error));
};
    ...
    <div className="personalize-main-container">
      ...
      <div className="personalize-thumbnails">
        {defaultImages.map(({ name, src }) => (
          <div key={name} 
          className={`personalize-thumbnail-image-container ${selectedImage === src ? "thumbnail-selected" : ""}`  }            
      ...
      <div>
          <div className={`personalize-thumbnail-image-upload-container`} onClick={handleUploadClick}>
      ...
      <button className='perzonalize-save-changes-btn'onClick={handleSaveChanges}>Save Changes</button>
      </div>
        </div>
        <Modal
        isOpen={modalIsOpen}
      ...
      >
        ...
        <button className="personalize-modal-btn"onClick={() => setModalIsOpen(false)}>Close</button>
      </Modal>
    </div>
  );
};
